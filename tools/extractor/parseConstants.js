...
const parseFile = async (filePath, startingDict = {}) => {
  ...
  lines.forEach((line) => {
    ...
    if (match !== null) {
      ...
      // skip rematch flags for now
      if (!macroName.match(/FLAG_REGISTERED_/)) {
        ...
        //console.log(`${macroName}: ${valueString}`)
        ;[...symbols].forEach(([symbol]) => {
          valueString = valueString.replace(symbol, output[symbol])
          //console.log(`New valueString: ${valueString}`)
        })
...
;(async () => {
  let output = {
    ...
    ...await parseFile(path.join(process.cwd(), 'include', 'constants', 'berry.h'))
  }
