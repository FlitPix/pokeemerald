    ...
    while (invalidMons != (1 << PARTY_SIZE) - 1) // All mons are invalid.
    {
        ...
        for (i = firstId; i < lastId; i++)
        {
                ...
                && i != *(gBattleStruct->monToSwitchIntoId + battlerIn2))
            {
                ...
                /* Possible bug (outside of Challenge Mode): this comparison gives the type that takes
                the most damage, when a "good" AI would want to select the type that takes the least damage.
                Unknown if this is a legitimate mistake or if it's an intentional, if weird, design choice */
                if (gArchipelagoOptions.isChallengeMode)
                {
                    if (bestDmg > typeDmg)
                    {
        ...
        if (bestMonId != PARTY_SIZE)
        {
            ...
            if (i != MAX_MON_MOVES || (gArchipelagoOptions.isChallengeMode && checkedAllMonForSEMoves && bestDmg <= TYPE_MUL_NOT_EFFECTIVE))
                return bestMonId; // Has both the typing and at least one super effective move.

            invalidMons |= gBitTable[bestMonId]; // Sorry buddy, we want something better.
            if (gArchipelagoOptions.isChallengeMode && invalidMons == (1 << PARTY_SIZE) - 1 && !checkedAllMonForSEMoves) // If we already checked all for a super effective move, then use the one with the best typing
            {
                invalidMons = 0;
