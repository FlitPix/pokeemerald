...
void ItemUseOutOfBattle_Bike(u8 taskId)
{
    ...
    else if (IsBikingDisallowedByPlayer() == 0)
    {
...
void CycleThroughRepels(void)
{
    ...
    while (gSpecialVar_Result == FALSE) {  // there are no more of the last-used repel type, checked in EventScript_RepelWoreOff
        gSpecialVar_Result = CheckBagHasItem(RepelCycle[i], 1);  // check if player has other kinds of repel, sorted from worst to best
