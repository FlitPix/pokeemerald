 ...
 * Task_NewGameBirchSpeech_WaitPressBeforeWallClock
 * Task_NewGameBirchSpeech_StartWallClock
 *  - Returns to CB2_NewGameBirchSpeech_ReturnFromWallClock when done
 * CB2_NewGameBirchSpeech_ReturnFromWallClock
 * Task_NewGameBirchSpeech_ReturnFromWallClockShowTextbox
 *
 * Task_NewGameBirchSpeech_ChooseYourPartner
 * Task_NewGameBirchSpeech_WaitForChooseYourPartnerToPrint
 * Task_NewGameBirchSpeech_WaitPressBeforeChooseStarter
 * Task_NewGameBirchSpeech_ChooseStarter
 *  - Returns to CB2_NewGameBirchSpeech_ReturnFromStarters when done
 * CB2_NewGameBirchSpeech_ReturnFromStarters
 * Task_NewGameBirchSpeech_ReturnFromStarterShowTextbox
 *
...
void CB2_NewGameBirchSpeech_ReturnFromWallClock(void);
void CB2_NewGameBirchSpeech_ReturnFromStarters(void);
...
static void Task_NewGameBirchSpeech_WaitPressBeforeWallClock(u8);
static void Task_NewGameBirchSpeech_StartWallClock(u8);
static void Task_NewGameBirchSpeech_ChooseYourPartner(u8);
static void Task_NewGameBirchSpeech_WaitForChooseYourPartnerToPrint(u8);
static void Task_NewGameBirchSpeech_WaitPressBeforeChooseStarter(u8);
static void Task_NewGameBirchSpeech_ChooseStarter(u8);
...
static void Task_NewGameBirchSpeech_ReturnFromWallClockShowTextbox(u8);
static void Task_NewGameBirchSpeech_ReturnFromStarterShowTextbox(u8);
...
static void Task_NewGameBirchSpeech_WaitForSpriteFadeInWelcome(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        else
        {
            ...
            StringExpandPlaceholders(gStringVar4, gText_Birch_Welcome_Emerald);
...
static void Task_NewGameBirchSpeechSub_InitPokeBall(u8 taskId)
{
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, PALETTES_BG, gArchipelagoOptions.introSpecies);
...
static void Task_NewGameBirchSpeech_ReshowBirchLotad(u8 taskId)
{
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        AddTextPrinterForMessage(TRUE);
        gTasks[taskId].func = Task_NewGameBirchSpeech_WhatTimeIsIt;
    }
}

static void Task_NewGameBirchSpeech_WhatTimeIsIt(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        ...
        if (!RunTextPrintersAndIsPrinter0Active())
        {
            StringExpandPlaceholders(gStringVar4, gText_Birch_WhatTimeIsIt);
            AddTextPrinterForMessage(TRUE);
            gTasks[taskId].func = Task_NewGameBirchSpeech_WaitForWhatTimeIsItToPrint;
        }
    }
}

static void Task_NewGameBirchSpeech_WaitForWhatTimeIsItToPrint(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
        gTasks[taskId].func = Task_NewGameBirchSpeech_WaitPressBeforeWallClock;
}

static void Task_NewGameBirchSpeech_WaitPressBeforeWallClock(u8 taskId)
{
    if ((JOY_NEW(A_BUTTON)) || (JOY_NEW(B_BUTTON)))
    {
        BeginNormalPaletteFade(PALETTES_ALL, 0, 0, 16, RGB_BLACK);
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartWallClock;
    }
}

static void Task_NewGameBirchSpeech_StartWallClock(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        FreeAllWindowBuffers();
...
static void Task_NewGameBirchSpeech_ChooseYourPartner(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        NewGameBirchSpeech_ClearWindow(0);
        StringExpandPlaceholders(gStringVar4, gText_Birch_ChooseYourPartner);
        AddTextPrinterForMessage(TRUE);
        gTasks[taskId].func = Task_NewGameBirchSpeech_WaitForChooseYourPartnerToPrint;
    }
}

static void Task_NewGameBirchSpeech_WaitForChooseYourPartnerToPrint(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
        gTasks[taskId].func = Task_NewGameBirchSpeech_WaitPressBeforeChooseStarter;
}

static void Task_NewGameBirchSpeech_WaitPressBeforeChooseStarter(u8 taskId)
{
    if ((JOY_NEW(A_BUTTON)) || (JOY_NEW(B_BUTTON)))
    {
        BeginNormalPaletteFade(PALETTES_ALL, 0, 0, 16, RGB_BLACK);
...
void CB2_NewGameBirchSpeech_ReturnFromWallClock(void)
{
    ...
    LoadPalette(&sBirchSpeechBgGradientPal[1], BG_PLTT_ID(0) + 1, PLTT_SIZEOF(8));
    ResetTasks();
    taskId = CreateTask(Task_NewGameBirchSpeech_ReturnFromWallClockShowTextbox, 0);
    gTasks[taskId].tTimer = 5;
    ScanlineEffect_Stop();
    ResetSpriteData();
    FreeAllSpritePalettes();
...
void CB2_NewGameBirchSpeech_ReturnFromStarters(void)
{
    ...
    LoadPalette(&sBirchSpeechBgGradientPal[1], BG_PLTT_ID(0) + 1, PLTT_SIZEOF(8));
    ResetTasks();
    taskId = CreateTask(Task_NewGameBirchSpeech_ReturnFromStarterShowTextbox, 0);
    gTasks[taskId].tTimer = 5;
    ScanlineEffect_Stop();
    ResetSpriteData();
    FreeAllSpritePalettes();
...
{
    return CreateMonPicSprite_Affine(gArchipelagoOptions.introSpecies, SHINY_ODDS, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}
...
static void Task_NewGameBirchSpeech_ReturnFromWallClockShowTextbox(u8 taskId)
{
    if (gTasks[taskId].tTimer-- <= 0)
    {
        NewGameBirchSpeech_ShowDialogueWindow(0, 1);
        gTasks[taskId].func = Task_NewGameBirchSpeech_ChooseYourPartner;
    }
}

static void Task_NewGameBirchSpeech_ReturnFromStarterShowTextbox(u8 taskId)
{
    if (gTasks[taskId].tTimer-- <= 0)
    {
        NewGameBirchSpeech_ShowDialogueWindow(0, 1);
