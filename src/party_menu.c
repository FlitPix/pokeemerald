...
void DisplayPartyMenuStdMessage(u32 stringId)
{
    ...
    if (stringId != PARTY_MSG_NONE)
    {
        ...
        if (stringId == PARTY_MSG_CHOOSE_MON)
        {
            ...
            {
                // check if the opponent is about to send out a new pokemon
                if (species >= NUM_SPECIES || species == SPECIES_NONE)
                {
                    // check if any opposing pokemon are on the field
                    ...
                    if (species >= NUM_SPECIES || species == SPECIES_NONE || gBattleTypeFlags & BATTLE_TYPE_DOUBLE)
                        stringId = PARTY_MSG_CHOOSE_MON_2; // no species on other side, use default text
                }
                if (stringId == PARTY_MSG_CHOOSE_MON)
...
static void Task_LearnedMove(u8 taskId)
{
    ...
    if (move[1] == 0)
    {
        ...
        if (item < ITEM_HM01 && !gArchipelagoOptions.reusableTms)
            RemoveBagItem(item, 1);
    }
    GetMonNickname(mon, gStringVar1);
    StringCopy(gStringVar2, gMoveNames[move[0]]);
